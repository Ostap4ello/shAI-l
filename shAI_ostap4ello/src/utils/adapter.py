...
def _http_req(method: str, url: str, payload: Optional[dict] = None) -> dict:
    data = None
    headers = {}
    ...
    try:
        ...
        raise RuntimeError(f"Failed to reach service at {url}: {e.reason}") from e
...
def _call_bash_script(
    ...
    logger.debug(f"Script {script_path} finished with code {return_code}.")
...
def run_ollama(context_length: int, gpus: str, name: str):
    ...
    logger.info(f"Ollama Docker container created successfully.")
    return result
...
def start_ollama(context_length: int, gpus: str, name: str, create: bool):
    ...
    result, _, stderr = _call_bash_script("ollama-docker.sh", ["--name", name, "begin"])

    if result != 0 and create:
        result, _, stderr = run_ollama(
            context_length=context_length, gpus=gpus, name=name
        )

    if result != 0:
        raise RuntimeError(f"Failed to start Ollama Docker service (create={create}). Stderr:\n{stderr}")
...
def stop_ollama(name: str, remove: bool):
    ...
    if remove:
        result, _, stderr = _call_bash_script(
            "ollama-docker.sh", ["--name", name, "kill"]
        )
    else:
        result, _, stderr = _call_bash_script(
            "ollama-docker.sh", ["--name", name, "stop"]
        )
    if result != 0:
        raise RuntimeError(f"Failed to stop Ollama Docker service (remove={remove}). Stderr:\n{stderr}")

    logger.info(
        f"Ollama Docker container {"removed" if remove else "stopped"} successfully."
    )
...
def wait_ollama_ready(
    ollama_url: str, timeout_sec: int = 10, interval_sec: float = 2.0
) -> None:
...
def rm_model(ollama_url: str, model_name: str) -> dict:
    ...
    return _http_req(
        "DELETE", f"{ollama_url.rstrip("/")}/api/delete", {"model": model_name}
    )
